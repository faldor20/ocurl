...
/*
 * Curl share interface support
 *
 * Exported functions are prefixed with caml_curl_share_
 */
...
static void raise_share_error(char const* msg)
{
  static const value* exception = NULL;
...
static void check_share_code(char const* func, CURLSHcode code)
{
  if (code != CURLSHE_OK) {
    char buf[100];
    snprintf(buf, sizeof(buf), "%s failed with code %d", func, code);
    raise_share_error(buf);
  }
}
...
value caml_curl_share_init(value unit)
{
  ...
  if (!h)
  {
    ...
    raise_share_error("caml_curl_share_init");
  }
...
{
  CAMLparam1(code);
  CAMLreturn(caml_copy_string(curl_share_strerror((CURLSHcode)Int_val(code))));
}
